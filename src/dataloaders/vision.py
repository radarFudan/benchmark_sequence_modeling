...
class CIFAR10C(SequenceDataset):
    ...
    def setup(self):
        ...
        ), "You must specify a corruption. Options are: " + str(
            sorted(
                [p.stem for p in self.data_dir.glob("*.npy") if not p.stem == "labels"]
            )
        )
        assert os.path.isfile(
            os.path.join(self.data_dir, f"{self.corruption}.npy")
        ), f"Corruption '{self.corruption}' does not exist. Options are: " + str(
            sorted(
                [p.stem for p in self.data_dir.glob("*.npy") if not p.stem == "labels"]
            )
        )
